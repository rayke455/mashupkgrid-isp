...
/** Where an agent invite leads: the agent chooses their email and password, then lands in the
 *  agent app. */
...
export default function AgentJoinPage() {
  ...
  useEffect(() => {
    apiFetch<Invite>(`/api/v1/agent-invite/${code}`)
      .then(setInvite)
  ...
  async function submit(e: FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await apiFetch<{ accessToken: string }>(`/api/v1/agent-invite/${code}`, { method: "POST", body: JSON.stringify({ email, password }) });
      ...
      <div className="mx-auto max-w-md">
        ...
        {invalid && (
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">It may have expired or already been used. Ask the ISP to send you a new invite, or sign in if you already set up your login.</p>
            <Link href="/login?next=/agent" className="mt-4 inline-block rounded-lg bg-sky-700 px-4 py-2 text-sm font-medium text-white">
              Sign in
            </Link>
          </div>
        )}
        ...
        {invite && (
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <p className="mt-1 text-sm text-slate-600">
              You are an agent for {invite.isp}. Set up your login to sell WiFi vouchers and take customers&apos; bill payments.
            </p>
