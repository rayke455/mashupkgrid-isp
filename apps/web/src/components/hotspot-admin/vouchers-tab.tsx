...
export function VouchersTab({ brand }: { brand: string }) {
  ...
  return (
    <div className="space-y-6">
      <Panel title="Make vouchers" description="Printable codes you sell for cash. Each one works once, for the time and limits below.">
        <form onSubmit={submit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="sm:col-span-2">
              <Label htmlFor="v-package">Based on</Label>
              <select id="v-package" className={selectClass} value={packageId} onChange={(e) => setPackageId(e.target.value)}>
                ...
                <option value="custom">Custom limits (no package)</option>
              </select>
            </div>
            <div>
              <Label htmlFor="v-count">How many</Label>
          ...
          {!usingPackage && (
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                <Label htmlFor="v-min">Lasts (minutes)</Label>
                <Input id="v-min" inputMode="numeric" value={minutes} onChange={(e) => setMinutes(e.target.value)} />
              </div>
              <div>
                <Label htmlFor="v-data">Data limit (MB)</Label>
                <Input id="v-data" inputMode="numeric" value={dataCap} onChange={(e) => setDataCap(e.target.value)} placeholder="Unlimited" />
              </div>
              <div>
                <Label htmlFor="v-dev">Devices</Label>
                <Input id="v-dev" inputMode="numeric" value={devices} onChange={(e) => setDevices(e.target.value)} />
              </div>
              <div>
                <Label htmlFor="v-down">Download (Mbps)</Label>
                <Input id="v-down" inputMode="decimal" value={downMbps} onChange={(e) => setDownMbps(e.target.value)} placeholder="No limit" />
              </div>
              <div>
                <Label htmlFor="v-up">Upload (Mbps)</Label>
                <Input id="v-up" inputMode="decimal" value={upMbps} onChange={(e) => setUpMbps(e.target.value)} placeholder="No limit" />
              </div>
              <div>
                <Label htmlFor="v-app">Unlocks</Label>
      ...
      >
        ...
        {isLoading ? (
          <p className="px-5 py-8 text-sm text-slate-400">Loading vouchers…</p>
        ) : vouchers.length === 0 ? (
